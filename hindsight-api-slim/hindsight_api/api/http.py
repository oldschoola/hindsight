...
class UpdateMemoryRequest(BaseModel):
    ...
    text: str | None = Field(
        ...
        "observations and links, and triggers re-consolidation. The previous "
        "text is preserved in the memory's history.",
    )
    state: str | None = Field(
        ...
        "pruned, embedding dropped) or 'valid' to revert. Reversible.",
    )
    reason: str | None = Field(
        default=None,
        description="Optional free-text reason recorded on the memory.",
    )

    @model_validator(mode="after")
    def _require_text_or_state(self) -> "UpdateMemoryRequest":
        if self.text is None and self.state is None:
            raise ValueError("Provide at least one of 'text' or 'state'.")
