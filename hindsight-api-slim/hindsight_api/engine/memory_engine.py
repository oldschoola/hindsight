...
class MemoryEngine(MemoryEngineInterface):
    ...
    async def purge_invalidated_memories(
        self,
        bank_id: str,
        *,
        ...
        """Permanently delete invalidated memory units (storage reclamation / GC).

        The irreversible second phase of curation: ``invalidate`` (reversible)
        soft-retires a memory and prunes its derived data; ``purge`` hard-deletes
        those soft-retired rows once they are past an optional retention window,
        reclaiming the row and any remaining storage. Only rows with
        ``state = 'invalidated'`` are affected — valid memories are never touched.

        Args:
            bank_id: Bank ID.
            older_than_days: Only purge rows invalidated at least this many days
                ago. None purges all invalidated rows regardless of age.
            request_context: Request context for authentication.
        ...
        if self._operation_validator:
            ...
            ctx = BankWriteContext(
                bank_id=bank_id, operation="purge_invalidated_memories", request_context=request_context
            )
            await self._validate_operation(self._operation_validator.validate_bank_write(ctx))
        ...
        async with acquire_with_retry(backend) as conn:
            async with conn.transaction():
                ...
                if older_than_days is not None:
                    cutoff_clause = " AND invalidated_at < now() - make_interval(days => $2::int)"
                    params.append(older_than_days)
                rows = await conn.fetch(
                    f"""
                    SELECT id FROM {fq_table("memory_units")}
                    WHERE bank_id = $1 AND state = 'invalidated'{cutoff_clause}
                    """,
                    *params,
                )
                purged_ids = [str(r["id"]) for r in rows]
                if purged_ids:
                    # Defensive: any observation still referencing a purged row is
                    # dropped (normally already gone from the invalidate cascade).
                    await self._delete_stale_observations_for_memories(conn, bank_id, purged_ids)
                    await conn.execute(
                        f"DELETE FROM {fq_table('memory_units')} WHERE id = ANY($1::uuid[])",
                        purged_ids,
                    )
        ...
        if purged_ids:
            # Deleting rows can orphan entities; let graph maintenance sweep them.
            try:
                await self.submit_async_graph_maintenance(bank_id=bank_id, request_context=request_context)
            except Exception as e:
                logger.warning(f"Failed to submit graph maintenance after purging memories in bank {bank_id}: {e}")

        return {"purged_count": len(purged_ids)}
    ...
    async def list_memory_units(
        ...
        *,
        ...
        Args:
            ...
            state: Optional curation-state filter ('valid' or 'invalidated'). When
                omitted, both valid and invalidated memories are returned so the
                invalidated ones remain visible/auditable (e.g. in a document view).
